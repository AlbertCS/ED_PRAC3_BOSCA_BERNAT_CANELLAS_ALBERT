...
public class Aplicacio {
				...
				if(plana.equals(frase)){	//Nova Plana
					...
					while(st.hasMoreTokens()){
						...
						if((a.equals(b))&&(eD.consultar(paraula2)==null)){
							...
							//sino no la tractem perquË no Ès repetitiva o perque es trobarepetida en la mateixa plana i linia
						}
					}
				}
				frase = f.readLine();
	...
	public static String nomCorrecte(Scanner teclat) {
		...
		System.out.println("Indica el nom del fitxer. Si no has creat cap, el nom que has de ficar Ès 'Text1'.");
	...
	public static String clauCorrecta(Scanner teclat) {
		...
		while(isOk){
			...
			if((nonum==false) && (i>0)){
				System.out.println("La clau que has introduit te algun car‡cter incorrecte. Indica la clau:");
				clau=teclat.nextLine();
				isOk=true;
			}
			else isOk=false;
